...
#include "E:/Xilinx/vhdl/hdp/nt/ieee/numeric_std/numeric_std.h"


static HSim__s6* IF0(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    extern HSim__s6 * createworkM_text_bench(const char*);
    HSim__s6 *blk = createworkM_text_bench(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_stopwatch_watch(const char*);
    HSim__s6 *blk = createWork_stopwatch_watch(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","TextBench", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("stopwatch/watch", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("stopwatch", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_text_bench(const char*);
        topvl = (HSim__s5*)createworkM_text_bench("TextBench");
