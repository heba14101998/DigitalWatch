...
va_list vap)
{
    extern HSim__s6 * createworkM_test_bench(const char*);
    HSim__s6 *blk = createworkM_test_bench(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","TestBench", (HSimInstFactoryPtr)IF0);
        ...
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_test_bench(const char*);
        topvl = (HSim__s5*)createworkM_test_bench("TestBench");
