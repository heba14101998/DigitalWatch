...
va_list vap)
{
    extern HSim__s6 * createworkM_t_b(const char*);
    HSim__s6 *blk = createworkM_t_b(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_alarm_behavioral(const char*);
    HSim__s6 *blk = createWork_alarm_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("alarm/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("alarm", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_t_b(const char*);
        topvl = (HSim__s5*)createworkM_t_b("TB");
