...
va_list vap)
{
    extern HSim__s6 * createworkM_t_e_s_t(const char*);
    HSim__s6 *blk = createworkM_t_e_s_t(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_collecting_behavioral(const char*);
    HSim__s6 *blk = createWork_collecting_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","TEST", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("collecting/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("collecting", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_t_e_s_t(const char*);
        topvl = (HSim__s5*)createworkM_t_e_s_t("TEST");
